...
struct BalancedGame {
	int result(vector <string> conflicts, int p, int q) {
		for(int i = 0;i < conflicts.size();i++){
			...
			for(int j = 0;j < N; j++){
				if(conflicts[i][j] == 'W')
					wins++;
				else if(conflicts[i][j] == 'L')
					loses++;
			}
			if(isUnbalanced(p, N, wins) || isUnbalanced(q, N, loses)){
				return i;
			} 
		}
		return -1;
	...
	bool isUnbalanced(int p, int N, int test){
		return test < ceil((N-1.0)*p/100);
	}
};
