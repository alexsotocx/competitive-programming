...
class Haar1D {
	...
	vector <int> transform(vector <int> data, int L) {
		return solve_transform(data, L, (int)(data.size()));
	}

	vector <int> solve_transform(vector <int> data, int L, int CL){
		if(L == 0)
			return data;
		vector <int> data2;
		vector <int> data1;
		for(int i = 0 ; i < CL; i += 2){
			data1.push_back(data[i] + data[i + 1]);
		...
		return solve_transform(data, L-1, CL/2);
